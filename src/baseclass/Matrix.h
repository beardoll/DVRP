...
public:
    ...
    // 得到row1-row2, col1-col2的子矩阵
    Matrix<T> getElemAtRowsByCols(int row1, int row2, int col1, int col2);
...
template <class T>
Matrix<T> Matrix<T>::getElemAtRowsByCols(int row1, int row2, int col1, 
        int col2){  // 得到row1-row2, col1-col2的子矩阵
    assert(row1>=0 && row1<rows && row2>=0 && row2<rows && row1<=row2);
    assert(col1>=0 && col1<cols && col2>=0 && col2<cols && col1<=col2);
